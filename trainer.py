...
import schedulers
import utils
from callbacks import TensorBoardImageSummary, TensorBoardWithLearningRate
from schedulers import SchedulerType


class Trainer:
    """
    Fits a given model to a datasets and configures learning rate schedulers
    and various callbacks

    :param name: Name of the model, used to build the target log directory
                if no explicit path is given
    :param log_dir_path: Path to the directory where the model and tensorboard
                summaries should be stored
    :param checkpoint_callback: Flag if checkpointing should be enabled.
                Alternatively a callback instance can be passed
    :param tensorboard_callback: Flag if information should be stored for
                tensorboard. Alternatively a callback instance can be passed
    :param tensorboard_images_callback: Flag if intermediate predictions
                should be stored in Tensorboard. Alternatively a callback
                instance can be passed
    :param callbacks: List of additional callbacks
    :param learning_rate_scheduler: The learning rate to be used. Either None
                for a constant learning rate, a `Callback` or a `SchedulerType`
    ...
    def __init__(self,
                 ...
                 checkpoint_callback: Optional[
                     Union[TensorBoard, bool]] = True,
                 tensorboard_callback: Optional[
                     Union[TensorBoard, bool]] = True,
                 tensorboard_images_callback: Optional[
                     Union[TensorBoardImageSummary, bool]] = True,
                 callbacks: Union[List[Callback], None] = None,
                 learning_rate_scheduler: Optional[
                     Union[SchedulerType, Callback]] = None,
                 **scheduler_opts,
                 ):
    ...
    def fit(self,
        ...
        train_dataset = train_dataset.map(
            utils.crop_labels_to_shape(prediction_shape)).batch(batch_size)
    ...
    def _build_callbacks(self,
        ...
        if isinstance(self.tensorboard_images_callback, Callback):
            ...
            if validation_dataset:
                tensorboard_image_summary = TensorBoardImageSummary(
                    "validation",
                    self.log_dir_path,
